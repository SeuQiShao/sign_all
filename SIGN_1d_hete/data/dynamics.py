...
class Kuramoto(MessagePassing):
    ...
    """
    def __init__(self, edge_index, edge_attr = None, init_noise = None, aggr = 'mean'):
        super(Kuramoto, self).__init__(aggr=aggr)
        self.aggr = aggr
        self.k = 1
        self.edge_index = edge_index
        nodes = edge_index.max() + 1
        if edge_attr is not None:
            self.edge_attr = edge_attr
        else:
            self.edge_attr = 1
        if init_noise:
            self.omega = torch.normal(mean=0.3, std=init_noise, size=(nodes, 1)).abs() 
        else:
            self.omega = 0.3
