...
import argparse
from model.utils import get_edgeindex_4_edges
...
class DatasetGenerator:
    ...
    def initialize_model(self, edge_index):
        ...
        if self.args.model_name == 'HeatDiffusion':
            ...
            self.model = Kuramoto(edge_index, init_noise=self.args.init_noise).to(self.args.device)
            self.para_k = self.model.k
    ...
    def generate_batch_sample(self):
        ...
        # 保存数据
        self.suffix = '{}_{}_{}_{}_{}_{}'.format(self.args.model_name, self.node_num, self.args.network, self.args.sample_interval, self.args.num_points, self.args.init_noise)
        # 创建模型名称的子目录
...
def get_args():
    ...
    parser.add_argument('--save_path', type=str, default='/data/scratch/acw802/SIGN_1d_hete/SIGN_data', help='Path to save the generated data')
    ...
    parser.add_argument('--network', type=str, choices=['random', 'power_law', 'small_world', 'from_file'], default='power_law', help='Type of graph topology to use')
    ...
    parser.add_argument('--init_noise', type=float, default=0.03, help='heterogenous factor for initial state values')
