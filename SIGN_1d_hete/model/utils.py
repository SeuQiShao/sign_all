...
#     # 合并所有节点并去重（比np.unique快30%）
#     all_nodes = np.concatenate([src_nodes, tgt_nodes])
#     unique_nodes = pd.unique(all_nodes)  # 使用pandas.unique加速
...
#     # 3. 映射边数据到新ID（约3秒，使用GPU加速）
#     # 使用torch.from_numpy避免内存复制
#     src_tensor = torch.from_numpy(src_nodes)
#     tgt_tensor = torch.from_numpy(tgt_nodes)
...
#     # 4. 构建edge_index（约0.5秒）
#     edge_index = torch.stack([mapped_src.cpu(), mapped_tgt.cpu()], dim=0)

#     # 6. 内存优化技巧（如需保留原始ID）
#     del edge_part, attr_part, src_nodes, tgt_nodes, all_nodes  # 及时释放内存
#     print('\n Grpah load finished! Edgeindex shape:', edge_index.shape, ' Node_num:', len(unique_nodes))
#     return edge_index, len(unique_nodes)
...
def functions(poly_p, poly_n, f_mask, c_mask, activate=False):
    ...
    fun_names = fun_lib(torch.empty((0,1)), poly_p, poly_n, device="cpu", activate=activate, names=True)
    coupled_names = coupled_fun_lib(None, None, poly_p, poly_n, device="cpu", activate=activate, names=True)
        # 合并名称列表
