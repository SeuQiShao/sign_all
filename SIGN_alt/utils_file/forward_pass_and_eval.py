...
def lasso_CF(args, batchs):
    ...
    model1 = linear_model.Lasso(alpha = 0.01, fit_intercept=True)
    model2 = linear_model.OrthogonalMatchingPursuit(n_nonzero_coefs = 15, fit_intercept=True)
    model1_0 = linear_model.Lasso(alpha = 0.01, fit_intercept=False)
    model2_0 = linear_model.OrthogonalMatchingPursuit(n_nonzero_coefs = 15, fit_intercept=False)
    model1.fit(x_data.cpu(), x_dot.cpu())
    model2.fit(x_data.cpu(), x_dot.cpu())
...
def forward_pass_and_eval(
    ...
    if save:
        true_data = target[:100,:,0].cpu().detach().numpy().T
        pred_data = output[:100,:,0].cpu().detach().numpy().T
        for i in range(true_data.shape[-1]):
            np.savetxt('true_{}_node_{}.csv'.format(args.ode_model,args.num_atoms), true_data, delimiter=',')
            np.savetxt('pred_{}_node_{}.csv'.format(args.ode_model,args.num_atoms), pred_data, delimiter=',')
