...
def R_syn(data, T_prime=None):
    """
    使用轨迹均值作为分界线，分别计算两个状态的同步度。

    参数：
    - data: Tensor, 形状 (N, T, F)，相位数据
    - T_prime: int, 计算前 T' 步 (若为 None，默认使用整个 T)

    返回：
    - (R_state1, R_state2): 每个状态的平均同步度
    """
    data = data % (2 * torch.pi)  # 确保数据是相位

    if T_prime is not None:
        data = data[:, :T_prime, :]

    phase_vector = torch.exp(data * 1.j)  # (N, T', F)

    # 计算轨迹均值
    mean_trajectory = data.mean(dim=(0, 1), keepdim=True)  # (1, 1, F)

    # 按照均值划分状态
    mask_state1 = data >= mean_trajectory  # (N, T', F)
    mask_state2 = data < mean_trajectory  # (N, T', F)

    def compute_R(mask):
        if mask.sum() == 0:  # 如果没有数据点，返回 NaN
            return float('nan')

        mean_phase = torch.exp(data[mask] * 1.j).mean(0)  # (F,)
        return mean_phase.abs().mean().item()  # 计算同步度

    def compute_R2():
        # R0 = compute_R2()
        # R_avg = min(R0, R_avg)
        mean_phase = torch.exp(data * 1.j).mean(0)  # (F,)
        return mean_phase.abs().mean().item()  # 计算同步度


    R_state1 = compute_R(mask_state1)  # 状态 1 的同步度
    R_state2 = compute_R(mask_state2)  # 状态 2 的同步度
    N1 = mask_state1.sum().item()
    N2 = mask_state1.sum().item()
    R_avg = (R_state1 * N1 + R_state2 * N2) / (N1 + N2) if (N1 + N2) > 0 else float('nan')
    R0 = compute_R2()
    R_avg = min(R0, R_avg)

    return R_avg, R_state1, R_state2



def R_syn_modified(U, n_clusters=2):
    """
    计算基于 KMeans 聚类的相位同步性指标
    
    参数:
    - U: 形状为 (N, T, F) 的数组，每个节点有 T 个时间步，F 维特征
    - n_clusters: 聚类类别数（默认2）

    返回:
    - overall_sync: 综合同步性指标
    - cluster_syncs: 每个类别的同步性
    - labels: 每个节点的聚类标签
    """
    N, T, F = U.shape

    # Step 1: 计算相位信息（对每个 F 分量进行 Hilbert 变换）
    phase_matrix = np.zeros((N, T, F))
    for f in range(F):
        analytic_signal = hilbert(U[:, :, f])  # 计算 Hilbert 变换
        phase_matrix[:, :, f] = np.angle(analytic_signal)  # 获取相位

    # Step 2: 聚类（使用展平后的数据）
    U_flattened = U.reshape(N, -1)
    kmeans = KMeans(n_clusters=n_clusters, n_init=10, random_state=42)
    labels = kmeans.fit_predict(U_flattened)
    ...
    for c in range(n_clusters):
        ...
        if len(cluster_indices) > 1:
            # 计算单位圆同步参数
            # mean_phase = torch.exp(phase_matrix[cluster_indices] * 1.j).mean(0)  # (F,)
            # sync =  mean_phase.abs().mean().item()  # 计算同步度
            phase_cluster = phase_matrix[cluster_indices]  # 该类的所有相位
            order_parameter = np.abs(np.mean(np.exp(1j * phase_cluster), axis=0))  # 计算同步性
            sync = np.mean(order_parameter)  # 取均值作为同步度量
            cluster_syncs[c] = sync
        else:
            cluster_syncs[c] = np.nan  # 处理极端情况

    # Step 4: 计算整体同步性
    overall_sync = np.nanmean(list(cluster_syncs.values()))

    order_parameter2 = np.abs(np.mean(np.exp(1j * phase_matrix), axis=0))  # 计算同步性
    sync2 = np.mean(order_parameter2)  # 取均值作为同步度量


    return overall_sync, cluster_syncs, np.mean(sync2)#labels
