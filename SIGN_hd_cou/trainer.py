...
def train():
    ...
    if args.load_folder == "":
        ...
        for epoch in range(args.epochs):
            ...
            expression = utils.functions(args.poly_p, args.poly_n, losses['wf'],  losses['wc'], activate=args.activate)[0]
    ...
    fianal_loss = forward_pass_and_eval.forward_pass_and_eval(args, decoder, batchs, epoch, c_mask=c_mask * soft_mask_c, f_mask=f_mask * soft_mask_f, save = args.save)
    ...
    with open(args.results_log, 'a+') as f:
        f.write(args.root + 'Seed: ' + str(args.seed) + 'dims: '+ str(args.k) + ' ' + 'r: '+ str(syn[0]) + ' '+ args.decoder +'\n')
        f.write(string)
        f.write('\n')
        f.write(expression)
        f.write('\n')
