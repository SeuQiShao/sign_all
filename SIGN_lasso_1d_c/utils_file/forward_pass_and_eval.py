...
def forward_pass_and_eval(
    ...
    if save:
        ...
        for i in range(true_data.shape[-1]):
            np.savetxt('true_{}_dim_{}.csv'.format(args.ode_model,args.num_atoms), true_data, delimiter=',')
            np.savetxt('pred_{}_dim_{}.csv'.format(args.ode_model,args.num_atoms), pred_data, delimiter=',')
