...
def generate_rdp_graph(n, d = 2):
    """生成随机点积图（RDPG）"""
    X = np.random.rand(n, d)  # 随机低秩隐变量
    P = X @ X.T  # 计算概率矩阵
    P = np.clip(P, 0, 1)  # 确保概率范围
    A = np.random.rand(n, n) < P  # 生成邻接矩阵
...
def generate_rpg(n, d=8, sparsity=0.5, scale=1.0):
    """生成扰动的高斯低秩稀疏网络（RPG）"""
    X = np.random.randn(n, d) * scale  # 调整scale来影响点的距离
    P = np.exp(-np.linalg.norm(X[:, None] - X, axis=2)**2)
    P *= sparsity  # 额外缩放概率
...
def generate_chung_lu(n):
    """ 生成 Chung–Lu 模型网络 """
    degrees = np.random.zipf(2.5, n)  # 生成幂律度分布
    degrees = degrees / degrees.sum()  # 归一化
...
def generate_rgm(n, radius):
    """生成随机几何图（RGM）"""
    G = nx.random_geometric_graph(n, radius)
    return G


def generate_sbm(n, k, p_in, p_out, sparsity_factor=0.1):
    """生成稀疏的随机块模型（SBM）"""
    sizes = [n // k] * k  # 每个社群的大小
    B = np.full((k, k), p_out)  # 组间连接概率
    np.fill_diagonal(B, p_in)  # 组内连接概率较高

    # 调整p_in和p_out值来实现稀疏性
    B *= sparsity_factor  # 使用 sparsity_factor 来调整连接概率，稀疏化
    
    G = nx.stochastic_block_model(sizes, B, seed=42)
    
    return G
# G_sbm = generate_sbm(n=100, k=3, p_in=0.3, p_out=0.05)
# nx.draw(G_sbm, node_size=20)
