...
def sample_observed_dynamics(dynamics, edge_index, ob_node_rate):
    """
    从给定的网络动力学数据中随机选择部分节点，并保留这些节点之间的边。
    
    参数：
    - dynamics: torch.Tensor, 形状为 (T, N, F)，表示动力学数据
    - edge_index: torch.LongTensor, 形状为 (2, E)，表示图的边索引
    - ob_node_rate: float, 保留的节点比例 (0-1)
    
    返回：
    - observed_dynamics: torch.Tensor, 形状为 (T, M, F)，仅包含保留节点的动力学数据
    - observed_edge_index: torch.LongTensor, 形状为 (2, E')，仅包含保留节点之间的边
    - observed_nodes: torch.Tensor, 形状为 (M,)，保留的节点索引
    """
    ...
    observed_nodes = torch.sort(observed_nodes).values  # 排序方便后续索引

    # 构建映射: 原索引 -> 新索引
    node_map = {old_idx.item(): new_idx for new_idx, old_idx in enumerate(observed_nodes)}

    # 过滤边，仅保留起点和终点都在 observed_nodes 中的边
...
def generate_missing_edges(solution: torch.Tensor, edge_index: torch.Tensor, edge_miss_rate: float):
    ...
    参数：
    - solution: T*N*F 的torch张量，表示动态系统的解。
    - edge_index: (2, E) 的torch张量，表示图的边结构。
    - edge_miss_rate: float，表示要随机移除的边比例。
    
    返回：
    - solution: 原始数据，不做修改。
    - masked_edge_index: (2, E') 的torch张量，表示部分边缺失后的图结构。
    """
    ...
    remove_indices = np.random.choice(num_edges, num_missing, replace=False)
    mask[remove_indices] = False
    
    # 生成缺失部分边的 edge_index
    masked_edge_index = edge_index[:, mask]
    
    return solution, masked_edge_index


# def fun_lib(x_i, poly_p, poly_n, device, activate=False, names=False):
#     with torch.no_grad():
#         # 如果只生成名称，则初始化空的 lib
#         lib = torch.ones(x_i.shape[0] * x_i.shape[1], 1).to(device) if not names else None
#         name_list = ["1"]  # 恒等基函数名称
#         # lib = torch.tensor([]).to(device) if not names else None
#         # name_list = []  # 恒等基函数名称

#         # Polynomial terms: 多项式部分
#         for i in range(1, poly_p):
#             if not names:
#                 lib = torch.cat((lib, torch.pow(x_i, i)), 1)
#             name_list.append(f"x^{i}")

#         # Fractional terms: 分数次幂
...
def fun_lib(x_i, poly_p, poly_n, device, activate=False, mask=None, names=False):
    ...
    generators.extend([
        ...
        lambda x: torch.tan(torch.clamp(x, -1.5, 1.5))  # 防止tan溢出
    ])
    name_list.extend(["sin(x)", "cos(x)", "tan(x)"])
    ...
    if activate:
        generators.extend([
            ...
            lambda x: x / (torch.abs(x) + 1)  # 安全版x/(x+1)
        ])
        name_list.extend(["sigmoid(x)", "tanh(x)", "x/(x+1)"])
...
# def coupled_fun_lib(x_i, x_j, poly_p, poly_n, device, activate=False, names=False):
#     with torch.no_grad():
#         # 如果只生成名称，则初始化空的 lib
#         lib = torch.tensor([]).to(device) if not names else None
#         name_list = []  # 恒等基函数名称
...
#                 lib = torch.cat((lib, torch.pow(x_j * x_i, i)), 1)
#             name_list.extend([f"x_j^{i}", f"(x_j - x_i)^{i}", f"(x_j * x_i)^{i}"])
...
#                 lib = torch.cat((lib, torch.pow(x_j * x_i, -i)), 1)
#                 lib = torch.cat((lib, torch.pow(x_j / x_i, -i)), 1)
#             name_list.extend([f"x_j^-{i}", f"(x_j - x_i)^-{i}", f"(x_j * x_i)^-{i}", f"(x_j / x_i)^-{i}"])
...
#             lib = torch.cat((lib, torch.cos(x_j) * x_i), 1)
#         name_list.extend([
#             "sin(x_j)", "cos(x_j)", "sin(x_j * x_i)", "cos(x_j * x_i)",
#             "sin(x_j - x_i)", "cos(x_j - x_i)", "sin(x_j) * x_i", "cos(x_j) * x_i"
#         ])

#         # Exponential terms: 指数函数
#         if not names:
#             lib = torch.cat((lib, torch.exp(x_j)), 1)
#             lib = torch.cat((lib, torch.exp(x_j * x_i)), 1)
#             lib = torch.cat((lib, torch.exp(x_j - x_i)), 1)
#             lib = torch.cat((lib, x_i * torch.exp(x_j)), 1)
#         name_list.extend(["exp(x_j)", "exp(x_j * x_i)", "exp(x_j - x_i)", "x_i * exp(x_j)"])
...
#                 lib = torch.cat((lib, x_j / (x_j + 1)), 1)
#                 lib = torch.cat((lib, x_i * x_j / (x_i * x_j + 1)), 1)
#                 lib = torch.cat((lib, (x_j - x_i) / (x_j - x_i + 1)), 1)
#                 lib = torch.cat((lib, x_j * x_i / (x_j + x_i + 1)), 1)
#             name_list.extend([
#                 "sigmoid(x_j)", "sigmoid(x_j * x_i)", "sigmoid(x_j - x_i)", "x_i * sigmoid(x_j)",
#                 "tanh(x_j)", "tanh(x_j * x_i)", "tanh(x_j - x_i)", "x_i * tanh(x_j)",
#                 "x_j / (x_j + 1)", "x_i * x_j / (x_i * x_j + 1)", "(x_j - x_i) / (x_j - x_i + 1)", 
#                 "x_j * x_i / (x_j + x_i + 1)"
#             ])
