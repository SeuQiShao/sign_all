...
def filter_denoise(args, data, process_var=1e-5, measurement_var=1e-2):
    """
    使用 KalmanFilter API 进行降噪
    :param data: (N, T, F) 形状的输入数据 (PyTorch Tensor)
    :param process_var: 过程噪声协方差
    :param measurement_var: 测量噪声协方差
    :return: 滤波后的数据 (N, T, F)
    """
    N, T, F = data.shape
    filtered_data = np.zeros((N, T, F))  # 存储滤波后的数据

    for i in range(N):  # 遍历每个样本
        filtered = savgol_filter(data[i, :, args.k].numpy(), window_length=24, polyorder=3, mode='interp')
        filtered_data[i,:,args.k] = filtered  # 存储结果

    return torch.tensor(filtered_data, dtype=data.dtype)  # 转回 PyTorch Tensor
