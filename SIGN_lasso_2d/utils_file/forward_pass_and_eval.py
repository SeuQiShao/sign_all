...
def lasso_CF(args, batchs):
    ...
    random_index = random.sample(list(np.arange(args.num_atoms)), int(nums_))
    ...
    for i in random_index:
        ...
        x_c = 0
        for j in x_neighbor:
            x_c += utils.coupled_fun_lib(x_i.reshape(-1,1), j.reshape(-1,1), ploy_p, ploy_n, device, activate=args.activate, tri = args.tri)
        if args.agg == 'mean':
            x_c = x_c/x_neighbor.shape[0]
        x_c = x_c * neighbor_coef
        x_dot0 = (data[i,:,args.k].diff(1)/t.diff()[0]).reshape(-1)
        x1 = utils.fun_lib(data[i,:-1,args.k].reshape(-1,1), ploy_p, ploy_n, device, activate=args.activate, tri = args.tri)
        x2 = utils.fun_fc_lib2(data[i,:-1,:].reshape(-1,args.dims), ploy_p, device)
        ####normalization
        x_data0 = torch.cat((x1[:,1:], x2, x_c), 1)
