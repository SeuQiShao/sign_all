...
class GSI_F(MessagePassing):
    def __init__(self, args):
        ...
        self.num_func_lib = 1 * (args.poly_n + 1) + 3 * args.poly_p
        if self.activate:
            self.num_func_lib += 3
        if self.tri:
            self.num_func_lib += 3
    ...
    def forward(self, t, x, batchs):
        ...
        wf_2 = self.wf_2
        neighbor = copy.deepcopy(batchs.neighbor)
        neighbor[:,[batchs.k]] = x
        out = self.update(x=x, wf_1 = wf_1, wf_2 = wf_2, neighbor = neighbor)
        return out




    def update(self, x, wf_1, wf_2, neighbor):
        with torch.no_grad(): 
            F_msg_f = utils.fun_lib(x, self.ploy_p, self.ploy_n, self.device, activate = self.activate, tri = self.tri)
            F_msg_fc = utils.fun_fc_lib3(neighbor, self.ploy_p, self.device)
            F_msg = torch.cat((F_msg_f, F_msg_fc), 1)
...
class GSI_C(MessagePassing):
    def __init__(self, args):
        ...
        self.num_coupled_fun_lib = 3 * (args.poly_p - 1) + 4 * (args.poly_n - 1) + 4 
        if self.activate:
            self.num_coupled_fun_lib += 12
        if self.tri:
            self.num_coupled_fun_lib += 8
    ...
    def message(self, x_i, x_j, edge_attr, wc_1, wc_2):
        ...
        with torch.no_grad(): 
            C_msg = utils.coupled_fun_lib(x_i, x_j, self.ploy_p, self.ploy_n, self.device, activate= self.activate, tri = self.tri)
...
class DGSIDecoder(nn.Module):
    def __init__(self, args):
        ...
        self.device = args.device
        self.num_func_lib = 1 * ((args.poly_n + 1) + 3 * args.poly_p)
        self.num_coupled_fun_lib = 1 * (3 * (args.poly_p - 1) + 4 * (args.poly_n - 1) + 4 )
        if self.activate:
            self.num_func_lib += 3
            self.num_coupled_fun_lib += 12
        if self.tri:
