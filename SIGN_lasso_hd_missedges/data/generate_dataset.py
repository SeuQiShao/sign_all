...
class DatasetGenerator:
    ...
    def generate_batch_sample(self):
        ...
        for _ in tqdm(range(self.args.sample_num)):           
            ...
            solution, masked_edge_index = generate_missing_edges(solution, edge_index, self.args.miss_edge_rate)
            # 将数据存储为 PyG Data 格式
            data = Data(
        ...
        # 保存数据
        self.suffix = '{}_{}_{}_{}_{}_{}'.format(self.args.model_name, self.node_num, self.args.network, self.args.sample_interval, self.args.num_points, self.args.miss_edge_rate)
        # 创建模型名称的子目录
...
def get_args():
    ...
    parser.add_argument('--miss_edge_rate', type=float, default=0.01, help='missed links rate')
