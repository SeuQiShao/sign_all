...
class GSICell(MessagePassing):
    def __init__(self, args):
        ...
        self.wf_2 = nn.Parameter(
            torch.cat((
                torch.rand(self.num_func_lib, 1) * 0.09 + 0.9,  # 生成 U(0.9, 0.99)
                torch.rand(self.num_func_lib, 1) * 0.09 + 0.01  # 生成 U(0.01, 0.1)
            )), 
            requires_grad=True
        )
        ########### C部分参数 #############
        ...
        self.wc_2 = nn.Parameter(
            torch.cat((
                torch.rand(self.num_coupled_fun_lib, 1) * 0.09 + 0.9,  # 生成 U(0.9, 0.99)
                torch.rand(self.num_coupled_fun_lib, 1) * 0.09 + 0.01  # 生成 U(0.01, 0.1)
            )), 
            requires_grad=True
        )
        # self.wc_2 = nn.Parameter(
