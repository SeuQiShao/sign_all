...
import argparse
from model.utils import get_edgeindex_4_edges, partition_graph_pyg, generate_missing_edges
...
class DatasetGenerator:
    ...
    def add_gaussian_noise_torch(self, X, snr_db):
        """
        给 (T, N, F) 形状的信号 X 添加 SNR 为 snr_db dB 的高斯白噪声
        """
        P_s = torch.mean(X**2)  # 计算信号功率
        P_n = P_s * 10**(-snr_db / 10)  # 计算噪声功率
        noise = torch.randn_like(X) * torch.sqrt(P_n)  # 生成噪声
        return X + noise  # 叠加噪声
    ...
    def generate_batch_sample(self):
        ...
        # 保存数据
        self.suffix = '{}_{}_{}_{}_{}_{}'.format(self.args.model_name, self.node_num, self.args.network, self.args.sample_interval, self.args.num_points, self.args.obnoise)
       # 创建模型名称的子目录
...
def get_args():
    ...
    parser.add_argument('--save_path', type=str, default='/data/scratch/acw802/SIGN_lasso_hd_obnoise/SIGN_data', help='Path to save the generated data')
    parser.add_argument('--obnoise', type=int, default=20, help='Observation noise')
