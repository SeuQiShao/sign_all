...
def train():
    ...
    fianal_loss = forward_pass_and_eval.forward_pass_and_eval(args, decoder, batchs, epoch, c_mask=c_mask * soft_mask_c0, f_mask=f_mask * soft_mask_f0, save = args.save)
...
if __name__ == "__main__":
    ...
    if args.t_basis:
        args.T_max_k = utils.generate_season(args, onebatchs.x[:,:,0].cpu().numpy(), onebatchs.t.cpu().numpy(), max_k=args.k_num)
        print(args.T_max_k)
    else:
        args.T_max_k = 0
    args.k = 0
