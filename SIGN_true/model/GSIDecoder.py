...
class GSI_C(MessagePassing):
    ...
    def message(self, x_i, x_j, edge_attr, wc_1, wc_2):
        ...
        with torch.no_grad(): 
            ...
            C_msg = C_msg[:,index]
        C_w = wc_1 * wc_2
        out = edge_attr * torch.mm(C_msg, C_w[index])
        return out
...
class DGSIDecoder(nn.Module):
    ...
    def single_step_forward(self, t, delta_t, batchs, step_x):

        x_dot = self.GSICell(t, step_x, batchs)

        return x_dot * delta_t + step_x
    ...
    def forward(self, t, batchs, c_mask =None, f_mask =None):
        ...
        for i in range(self.time_stamp - 1):
            if i%self.teacher == 0:            
                ...
                step_x = out[-1]
            out.append(self.single_step_forward(t[[i]],torch.diff(t)[0], batchs, step_x))
