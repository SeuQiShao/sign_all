...
# def fun_fc_lib3(x, ploy_p, device):
#     with torch.no_grad(): 
...
#             lib = torch.cat((lib, torch.pow(x[:,[2]], i)), 1)
#         lib = torch.cat((lib, x[:,[0]] * x[:,[1]]), 1)
#         lib = torch.cat((lib, x[:,[0]] * x[:,[2]]), 1)
#         lib = torch.cat((lib, x[:,[2]] * x[:,[1]]), 1)
...
def fun_lib(x_i, ploy_p, ploy_n, device, activate = False):
    with torch.no_grad(): 
        #Consitant:1
        ...
        # ###########clamp###############3#######
        lib = torch.clamp(lib, min=-100000, max=100000)
        ###########negetive part################
        # nlib = -lib
        # lib = torch.cat((lib, nlib), 1)
    return lib 
        
def coupled_fun_lib(x_i, x_j, ploy_p, ploy_n, device, activate = False):
    with torch.no_grad(): 
        #Consitant: 1
...
def generate_season(args, x, t, max_k = 10, norm = False):
    if norm:
...
def t_fun_lib(t, T_max_k, device):
    with torch.no_grad(): 
        t = t.unsqueeze(1)
        lib = torch.tensor([])
        lib = lib.to(device)
        for i in range(1,6):
            lib = torch.cat((lib, t ** i), 1)
        for i in T_max_k:
            lib = torch.cat((lib, torch.sin(2*torch.pi*t/i)), 1)
            #lib = torch.cat((lib, torch.sign(torch.sin(t/(2 * i + 1) * 2* torch.pi))), 1)
        for i in T_max_k:
            lib = torch.cat((lib, torch.cos(2*torch.pi*t/i)), 1)
            #lib = torch.cat((lib, torch.sign(torch.cos(t/(2 * i + 2) * 2* torch.pi))), 1)
        lib = torch.clamp(lib, min=-100000, max=100000)
    return lib
